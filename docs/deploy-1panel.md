...
默认端口为 `1841`，这个端口很少与其他服务冲突，**通常不需要修改**。

如果 `1841` 确实被其他项目占用了，可以编辑 `.env.docker` 修改 `APP_PORT`：

```env
APP_PORT=9090  # 宿主机映射端口（改成任意未占用的端口）
PORT=1841      # 容器内端口（不需要改）
```

然后重启：`docker compose up -d`

> **注意**：修改端口后，1Panel 反向代理的目标地址也要同步修改（如 `http://127.0.0.1:9090`）。
